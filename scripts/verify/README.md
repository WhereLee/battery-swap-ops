...
| batch43 | `_c40_ui_negative_control.ps1`（+ `.mjs`） | **界面门禁可红性实证**（批次43）：先测干净基线（判据必须为假）→ 注入一类缺陷 → 判据必须为真 → 移除后必须重新测得干净（证明探针"对注入反应"而非"脏了就粘住"）。四类注入：标签对比度（`--el-tag-text-color:#eee`）、单元格裁切（首列限宽 40px）、页面横向溢出（3000px 块）、表头错列（表头右移 30px）。**用门禁自己的判据代码**（`import { probeExpression }`，不复制不重写） | PASS 12/12（首版：四类注入；**现已扩到七类 = 21/21，见下行 batch43 补记**）（70 个标签/最差 1.05:1、20 个裁切、溢出 true、11 列错位、**空作用域被拒**；负控首跑即发现门禁盲区 → 见批记录 §三/§七）。配套 `_c38` 新增"作用域非空"自检：每个测量作用域必须至少看到 1 个元素，否则判 FAIL（此前的风险是"作用域选错→测了个空集→零问题→看起来全绿"） |
| batch43 补记 | 同 `_c38_ui_probe`（扩面，无新脚本）+ `_c40_ui_negative_control` | **界面门禁可红性实证（续）：七类注入** —— 在原有四类之外补三类：空作用域（指向新建空 div）、**告警变体对比度**（注入 EP 原色 `#e6a23c` 到 warning 浅底）、**表单校验红字对比度**（注入 EP 原色 `#f56c6c`）。新增 `setup`/`prepare`/`after` 钩子：C6/C7 需要**未登录**的登录表单（路由守卫会把已登录会话的 `/login` 弹回 `/dashboard`），故在一次性 profile 里删 token key，C7 跑完再登回来。仍然**用门禁自己的判据代码**（`import { probeExpression, alertMatrixExpression, ALERT_MATRIX_SCOPE }`） | PASS **21/21**（含 C6 告警矩阵 1/10 最差 **2.04:1**、C7 校验红字 1/8 最差 **2.9:1**、还原后重新干净） |
| batch43 补记 | 同 `_c38_ui_probe`（扩面，无新脚本） | **界面门禁补掉本批自己申报的三个盲区**：① **作用域指纹**——页面作用域必须含 `.page-title`、弹窗作用域签名必须含自身标题（`titleAny` 容纳退款/冲正动态标题），把"非空"升级为"是它"；② **弹窗可达性**——`preStep` 先点"建议单"页签（此前"驳回建议单"那条 SKIP 其实是**少点了一次**，不是"没数据"），修完立刻量到计数器 3.08:1；③ **校验态通道**——登录表单**空提交**才看得见 `.el-form-item__error`，并断言**零请求**（CDP 计数 `/admin/auth/login`）；④ **告警变体矩阵**——把登录页真实告警克隆 5 类×明/暗共 10 个变体，用 `probeExpression` 量（`alertMatrixExpression` 已导出给负控复用）；⑤ 新增 `worstText`：报告"最紧一处"而不只是"零问题" | PASS 10 页 + **2** 弹窗 + 校验态 + 10 变体 hardFailures=0（1970 个文字元素 / 270 标签 / **全轮最紧 4.70:1**）。据此修 3 处真缺陷：计数器 **3.08→6.1:1**（EP `.el-input .el-input__count` 特异性坑）、校验红字 **2.90→8.24:1**、登录页告警文案 **2.80:1**（`--el-color-info` 当文字色，第三个同型组件） |
