...
合计 **54 个剧本与验证脚本**（`_g` 主线 ×8 + `_c` 能力 ×41 + `_p03` 容量 ×3 + `_b18` 防回归 ×1 + Python 契约客户端 ×1，
