...
- 实机剧本 56 个（`scripts/verify/README.md` 总索引；batch1-43 全 PASS，含双实例 `_c29`、异构设备端 `_c30`、运维 Agent `_c31`、BFF 视图与数据权限 `_c32`、分页回归网 `_c33`、页面级契约 `_c34`、**真实浏览器验收 `_c35`**、限流 XFF 绕过 `_c36`、**布局/对比度探针 `_c38`**、**索引/访问路径审计 `_c39`**、**门禁可红性负控 `_c40`/`_c41`**）；另含两处**门禁可红性实证**（CI 集成测试红探针、事务自调用守卫红探针）
- **门禁可红性（negative control，批次43）**：给界面门禁与索引审计各做"注入缺陷→必须判红→还原→必须判绿"的实证——
  `_c40` 七类注入全部检出（标签对比度 70 个/最差 1.05:1、裁切 20 个、横向溢出、表头错列 11 列、**空作用域被拒**、
  **告警变体对比度**（还原 EP 原色 → 变体矩阵 1/10 最差 2.04:1）、**表单校验红字对比度**（还原 `#f56c6c` → 1/8 最差 2.9:1））**21/21**；
  `_c41` 丢掉访问路径后精确复现批次40 缺陷签名（`type=ALL key=NULL rows=7875`）9/9。
  **负控首跑即发现门禁盲区**：表头检查只比外层盒，子元素 transform 能躲过 → 判据已加强（内外层盒都算），
  加强后真实页面仍 `0 misaligned`（89 列）无假阳性
...
- 管理台可访问性（实机测量，`_c38`）：**10 条路由 1936 个文字元素 + 270 个标签对比度全部达标**
  ...
  标签 2.04–3.08:1、**占位符 2.30:1**、**输入计数器 3.08:1**、**表单校验红字 2.90:1**、**告警文案 2.80:1**；
  已做令牌/组件级修正，标签区间 6.00–8.57:1、占位符 4.70:1；**全轮最紧一处 4.70:1**）、
  89 个表头零错列、零裁切、零横向溢出；另覆盖**弹窗**（2 个实测 / 2 个因 `allowedActions` 缺失记 SKIP 带原因）、
  **表单校验态**（登录空提交触发 `.el-form-item__error`，并断言"零请求"——失败校验不得到达服务端）、
  **告警变体矩阵**（真实告警克隆 5 类×明/暗 10 个变体，用同一套测量码；`0 below, worst 6:1`）
...
- 前端浏览器门禁（本地，需本机 Chrome + 平台在跑）：`_c34` 契约 59/59、`_c35` 走查 31/31（含整页截图）、`_c38` 布局/对比度 10 页 + 2 弹窗 + 校验态 + 10 告警变体 hardFailures=0、`_c40` 界面负控 21/21、`_c39` 索引审计 15/15（含无界读守类，需本机 MySQL）、`_c41` 索引负控 9/9
