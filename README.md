...
- 测试：**508/508**（契约 4 + 平台 444 + 模拟器 31 + Agent 29，Skipped 0）；JaCoCo 门槛 server 65% / sim 55% / contract 70% / agent 65%，CI `mvn verify` 强制（实测 server 71.5% / agent 86.2%）；前端门禁 `vue-tsc --noEmit` + `vite build`（CI `frontend` job）；集成测试 `*IT` 由 failsafe 在 CI（Docker）跑，本地 `mvn test` 不受影响
