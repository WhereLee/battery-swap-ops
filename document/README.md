...
### 根目录（入口型手册）
- `Git与CI与云部署-操作手册.md`（**照做可用**：§0 六条常用命令 / §1 Git（remote 实况、`-F` 中文提交、push 假失败、入仓红线）
  / §2 CI（两 job 逐步、6 项门禁、本地等价命令、看 CI 的正确写法、门禁有效性曾被探针实证）
  / §3 服务器（连接与铁律、布局与 env 键名、三种动作：全量/增量/只读探测、冒烟模板、备份恢复、云上差异表）
  / §4 一次完整块循环）
